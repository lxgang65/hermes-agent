...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "USE THIS ONLY when:\n"
        "- The user explicitly says 'we did this before', 'remember when', 'last time', 'as I mentioned'\n"
        "- The user asks to resume, recover, or browse a previous conversation\n"
        "- The user asks 'what did we do about X?' or 'how did we fix Y?'\n"
        "- Current memory/files are insufficient and the answer truly depends on prior session content\n\n"
        "Do not use this for simple chat, current status checks, current configuration, current "
        "workspace paths, or questions answerable from active context/files. Keyword search uses "
        "auxiliary LLM summarization and can be slow; no-query recent-session browsing is the fast mode.\n\n"
